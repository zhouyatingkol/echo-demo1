...
async function deploy() {
    try {
        ...
        // 计算总费用 (都转为 string 避免 BigInt 混合类型问题)
        const gasEstimateStr = gasEstimate.toString();
        const gasPriceStr = gasPrice.toString();
        const totalCost = BigInt(gasEstimateStr) * BigInt(gasPriceStr);
        ...
        // 创建交易 (gasEstimate 是 BigInt，需要转换)
        ...
        fs.writeFileSync(
            'deployment-v2-web3.json',
            JSON.stringify(deploymentInfo, null, 2)
        );
        
        console.log('\n💾 部署信息已保存到 deployment-v2-web3.json');
        ...
        if (error.message.includes('replacement transaction')) {
            console.error('💡 提示: 有 pending 交易，请等待或提高 Gas Price');
        }
        throw error;
    }
}
