...
import { exists, getAllFilesMatching, isDirectory, readdir, readJsonFile, } from "@nomicfoundation/hardhat-utils/fs";
 ...
 */
export async function getTemplates(templatesDir) {
    ...
    const templates = await Promise.all(pathsToTemplates.map(async (dirName) => {
        ...
        assertHardhatInvariant(await exists(pathToPackageJson), `package.json for template ${name} is missing`);
        const packageJson = await readJsonFile(pathToPackageJson);
        const files = await getAllFilesMatching(pathToTemplate, (f) => {
            ...
            if (path.relative(pathToTemplate, f).split(path.sep)[0] === "node_modules") {
                return false;
            }
            return true;
