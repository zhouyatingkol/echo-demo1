 ...
 */
export function getDevDependenciesInstallationCommand(packageManager, dependencies) {
    const packageManagerToCommand = {
    ...
    command.push(...dependencies.map((d) => {
 ...
 */
export async function installsPeerDependenciesByDefault(workspace, packageManager, version, config) {
    switch (packageManager) {
        case "npm":
            const npmVersion = await getVersion(workspace, "npm", version);
            const legacyPeerDeps = await getFromConfig(workspace, "npm", "legacy-peer-deps", config);
        ...
        case "pnpm":
            ...
            const autoInstallPeers = await getFromConfig(workspace, "pnpm", "auto-install-peers", config);
...
async function getVersion(workspace, packageManager, version) {
...
async function getFromConfig(workspace, packageManager, key, config) {
    if (config !== undefined) {
        return config[key];
    }
    try {
