...
import { hexStringToBigInt, numberToHexString, } from "@nomicfoundation/hardhat-utils/hex";
...
export class AutomaticGasPriceHandler {
    ...
    async handle(jsonRpcRequest) {
        ...
        let maxFeePerGas = typeof tx.maxFeePerGas === "string"
            ? hexStringToBigInt(tx.maxFeePerGas)
            : suggestedEip1559Values.maxFeePerGas;
        const maxPriorityFeePerGas = typeof tx.maxPriorityFeePerGas === "string"
    ...
    async #getGasPrice() {
        ...
        assertHardhatInvariant(typeof response === "string", "response should return a string");
        return hexStringToBigInt(response);
    }
    async #suggestEip1559FeePriceValues() {
        if (this.#nodeSupportsEIP1559 === undefined) {
            const block = await this.#provider.request({
                method: "eth_getBlockByNumber",
                params: ["latest", false],
            });
            assertHardhatInvariant(isObject(block), "block should be a non null object");
            this.#nodeSupportsEIP1559 =
                "baseFeePerGas" in block && block.baseFeePerGas !== undefined;
        }
        if (this.#nodeHasFeeHistory === false ||
        ...
        try {
            const response = await this.#provider.request({
                ...
                params: [
                    "0x1",
                    "latest",
                    [_a.EIP1559_REWARD_PERCENTILE],
                ],
            });
            assertHardhatInvariant(typeof response === "object" &&
                response !== null &&
                "baseFeePerGas" in response &&
                "reward" in response &&
                Array.isArray(response.baseFeePerGas) &&
                Array.isArray(response.reward), "response should be an object with baseFeePerGas and reward properties");
            ...
            if (maxPriorityFeePerGas === 0n) {
                try {
                    ...
                    assertHardhatInvariant(typeof suggestedMaxPriorityFeePerGas === "string", "suggestedMaxPriorityFeePerGas should be a string");
                    maxPriorityFeePerGas = hexStringToBigInt(suggestedMaxPriorityFeePerGas);
                }
                catch {
            ...
            return {
                ...
                // next N blocks here.
                maxFeePerGas: (hexStringToBigInt(response.baseFeePerGas[response.baseFeePerGas.length - 1]) *
                    9n **
                        (_a.EIP1559_BASE_FEE_MAX_FULL_BLOCKS_PREFERENCE -
                            1n)) /
                    8n **
                        (_a.EIP1559_BASE_FEE_MAX_FULL_BLOCKS_PREFERENCE -
                            1n),
...
_a = AutomaticGasPriceHandler;
//# sourceMappingURL=automatic-gas-price-handler.js.map