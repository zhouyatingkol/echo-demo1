...
class WalletManager {
    ...
    isCorrectChain() {
        // TODO: 根据实际部署的链修改
        // 目前支持：1=以太坊主网, 5=Goerli测试网, 1337=本地测试
        const supportedChains = [1, 5, 1337, 31337];
        ...
        try {
            ...
            if (switchError.code === 4902) {
                try {
                    ...
                    this.showError('添加网络失败: ' + addError.message);
                }
            }
        }
    }
    ...
    async purchaseLicense(assetId, licenseType, usageType, params) {
        ...
        try {
            // TODO: 部署合约后更新此地址
            // 当前为占位符，需要在合约部署后替换
            const LICENSE_CONTRACT_ADDRESS = localStorage.getItem('license_contract_address') || '0x0000000000000000000000000000000000000000';
            
            if (LICENSE_CONTRACT_ADDRESS === '0x0000000000000000000000000000000000000000') {
                this.showError('合约尚未部署，请联系管理员');
                return null;
            }
